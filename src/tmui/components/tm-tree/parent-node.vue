...
const flatlist = computed(() =>
  treeFlat(props.data["children"] ?? [], props.fieldNames.id)
);
//所有数据父节点
...
let sletctedIdArray = TreeParentIds.value.filter(
  (el: string | number) => el == nodeData.value.id
);
...
function setExpaned() {
  ...
  //如果差集长度大于0表示没有全部选中。
  //排除本身 。
  let ar: Set<string | number> = new Set([
    ...TreePareantSelectedIds.value,
    ...TreeParentIds.value,
  ]);
