<template>
  <view
    class="flex"
    :class="[
      _disabled ? props.disabledClass : '',
      tmCheckedBoxDir == 'row' ? 'flex-row' : '',
      tmCheckedBoxDir == 'customCol' ? 'flex-1' : '',
    ]"
  >
...
function vailChecked(val?: Array<string | number | boolean>) {
  ...
  let val_self: Array<string | number | boolean> =
    typeof val === "undefined" ? _groupCheckedVal.value : val;
