<template>
  ...
  >
    ...
    >
      ...
      >
        <view
          class="flex flex-row nowrap nonvue fulled"
          :class="[_align]"
        ...
        >
          <view :id="tabsid + item.key" v-for="(item, index) in cacheTabs" :key="index">
            ...
            >
              <view
                :style="[
                  props.itemWidth > 0 ? { width: props.itemWidth + 'rpx' } : {},
        ...
        <view
          ...
          :style="{
            ...
            backgroundColor: props.showTabsLineAni
              ? store.tmStore.dark
                ? '#616161'
                : '#ebebeb'
              : '',
      ...
      >
        <view :id="tabsid + item.key" v-for="(item, index) in cacheTabs" :key="index">
          ...
          >
            ...
            >
              <view
                class="flex flex-row flex-center px-20"
                :style="{ height: props.itemHeight - 20 + 'rpx' }"
              >
                <tm-icon
                  :userInteractionEnabled="false"
                  v-if="item.icon"
...
const activeIndex = computed(() =>
  cacheTabs.value.findIndex((el) => el.key == _active.value)
);
let ctxLeft = 0;
let ctxTop = 0;
